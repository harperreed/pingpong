...
impl TuiState {
    ...
    pub fn update_clocks(&mut self) {
        ...
        self.chicago_time = Central.from_utc_datetime(&Utc::now().naive_utc());
    }

    pub fn cycle_animation(&mut self) {
        self.animation_type = match self.animation_type {
            AnimationType::Plasma => AnimationType::Globe,
            AnimationType::Globe => AnimationType::BouncingLogo,
            AnimationType::BouncingLogo => AnimationType::Matrix,
            AnimationType::Matrix => AnimationType::Dna,
            AnimationType::Dna => AnimationType::Waveform,
            AnimationType::Waveform => AnimationType::Plasma,
        };
    }

    pub fn toggle_time_format(&mut self) {
        self.use_24_hour_format = !self.use_24_hour_format;
    }

    pub fn toggle_lore_visibility(&mut self) {
        self.show_lore = !self.show_lore;
    }
}
...
impl TuiApp {
    ...
    pub async fn draw(&mut self, stats: &HashMap<String, PingStats>) -> anyhow::Result<()> {
        ...
        let local_time = self.state.local_time;
        let chicago_time = self.state.chicago_time;
        let use_24_hour = self.state.use_24_hour_format;
        let show_lore = self.state.show_lore;
        ...
        if event::poll(Duration::from_millis(50))? {
            if let Event::Key(key) = event::read()? {
                match key.code {
                    ...
                    KeyCode::Char('v') => {
                        self.state.cycle_animation();
                    }
                    KeyCode::Char('p') => {
                        self.state.toggle_time_format();
                    }
                    KeyCode::Char('l') => {
...
fn render_main(
    ...
    // Split left side conditionally based on lore visibility
    if show_lore {
    ...
    render_status_bar(f, outer_chunks[1], local_time, chicago_time, use_24_hour);
}
...
fn render_animation_window(
    ...
    let (mut animation_art, title) = match animation_type {
        AnimationType::Plasma => {
            let art = generate_plasma_animation(
                animation_time,
                area.width as usize,
        ...
        AnimationType::Globe => {
            let art =
                generate_globe_animation(animation_time, area.width as usize, area.height as usize);
            (art, format!(" Digital Earth - RTT: {:.1}ms ", avg_rtt))
        }
        AnimationType::BouncingLogo => {
            let art = generate_bouncing_rtt_animation(
                bounce_pos,
                area.width as usize,
                area.height as usize,
                avg_rtt,
        ...
        AnimationType::Matrix => {
            let art = generate_matrix_animation(
                animation_time,
                area.width as usize,
                area.height as usize,
                avg_rtt,
        ...
        AnimationType::Dna => {
            let art = generate_dna_animation(
                animation_time,
                area.width as usize,
                area.height as usize,
                avg_rtt,
        ...
        AnimationType::Waveform => {
            let art = generate_waveform_animation(
                animation_time,
                area.width as usize,
                area.height as usize,
                avg_rtt,
...
fn generate_plasma_animation(time: f64, width: usize, height: usize) -> String {
    ...
    for y in 0..effective_height {
        ...
        for x in 0..effective_width {
            ...
            let turbulence =
                ((x as f64 * 0.05).sin() * (y as f64 * 0.07).cos() + time * 0.5).sin() * 0.3;

            // Combine all waves with different weights
            let intensity =
                (wave1 * 2.0 + wave2 * 1.8 + wave3 * 1.5 + wave4 * 1.2 + wave5 * 0.8 + turbulence)
                    * 0.8;

            // Map to character index (0-8)
            let char_index = ((intensity + 3.0) * 1.33).clamp(0.0, 8.0) as usize;
    ...
    for node in 0..num_nodes {
        ...
        let node_x = ((node_time * 0.7).sin() * (effective_width as f64 - 10.0) / 2.0
            + effective_width as f64 / 2.0) as usize;
        let node_y = ((node_time * 0.9 + node as f64).cos() * (effective_height as f64 - 6.0) / 2.0
            + effective_height as f64 / 2.0) as usize;
    ...
    for y in 0..effective_height {
        ...
        for x in 0..effective_width {
            ...
            if distance <= radius as f64 {
                ...
                let land_probability =
                    (continent_noise1 + continent_noise2 * 0.7 + continent_noise3 * 0.5) * 0.6;
                ...
                let terminator_blend = (day_night * 3.0).clamp(-1.0, 1.0);
                ...
                let char_to_use = if has_clouds {
                    let cloud_intensity =
                        ((cloud_noise + 1.0) * 4.0) as usize % cloud_patterns.len();
                    cloud_patterns[cloud_intensity]
                } else if land_probability > 0.1 {
                    // Land features based on latitude and terrain type
                    let terrain_type = ((latitude.abs() * 2.0 + longitude * 1.5) as usize)
                        % continent_layers.len();
                    let elevation = ((land_probability + 1.0) * 4.0) as usize
                        % continent_layers[terrain_type].len();
                    ...
                    let wave_intensity = ((distance / radius as f64 + time) * 4.0) as usize
                        % ocean_layers[ocean_type].len();
                ...
                let char_to_use =
                    if atmo_distance < 1.0 && aurora_effect > 0.8 && latitude.abs() > 0.6 {
                        // Aurora at poles
                        let aurora_chars = ['◉', '⚡', '✦', '◯', '●'];
                        ...
                        // Normal atmosphere
                        let atmo_intensity =
                            (atmo_distance * 4.0) as usize % atmosphere_chars.len();
...
fn generate_bouncing_rtt_animation(
    bounce_pos: (f64, f64),
    width: usize,
    height: usize,
    avg_rtt: f64,
...
fn generate_matrix_animation(time: f64, width: usize, height: usize, avg_rtt: f64) -> String {
    ...
    let speed_multiplier = if avg_rtt < 50.0 {
        2.0
    } else if avg_rtt < 150.0 {
        1.5
    } else {
    ...
    for x in 0..num_columns {
        ...
        if has_stream {
            ...
            let stream_y_offset = (time * speed_multiplier * stream_speed
                + column_phase * effective_height as f64)
                % (effective_height as f64 + stream_length as f64 * 2.0);
            ...
            for i in 0..stream_length {
                ...
                if y >= 0 && y < effective_height as isize {
                    ...
                    let char_seed =
                        (column_base_seed + i * 11 + time_factor / 3) % matrix_chars.len();
    ...
    if avg_rtt > 150.0 {
        ...
        let num_glitches =
            (effective_height as f64 * effective_width as f64 * glitch_intensity * 0.02) as usize;
        ...
        for _ in 0..num_glitches {
            ...
            if glitch_y < result.len() {
                ...
                if glitch_x < chars.len() {
                    ...
                    let glitch_chars = ['▒', '░', '·', '?', '‾'];
                    let glitch_seed =
                        ((time * 1.0) as usize + glitch_x + glitch_y) % glitch_chars.len();
    ...
    if avg_rtt < 15.0 && ((time * 0.3) as usize % 25) < 4 {
        ...
        if center_y < result.len() && effective_width > message.len() {
            ...
            for clear_x in
                start_x.saturating_sub(1)..=(start_x + message.len()).min(effective_width - 1)
            {
...
fn generate_dna_animation(time: f64, width: usize, height: usize, avg_rtt: f64) -> String {
    ...
    let center_x = effective_width / 2;
    let helix_width = (effective_width / 4).clamp(3, 8);
...
fn generate_waveform_animation(time: f64, width: usize, height: usize, avg_rtt: f64) -> String {
    ...
    for x in 0..effective_width {
        ...
        let y_pos =
            (center_y as isize + combined_wave).clamp(0, effective_height as isize - 1) as usize;
    ...
    if effective_height > 3 {
        let signal_strength = if avg_rtt < 50.0 {
            "STRONG"
        } else if avg_rtt < 150.0 {
            "MEDIUM"
        } else {
...
fn generate_connection_failure_overlay(
...
fn render_status_bar(
    ...
    let time_format = if use_24_hour {
        "%H:%M:%S"
    } else {
        "%I:%M:%S %p"
    };
    let local_time_str = local_time.format(time_format).to_string();
    let chicago_time_str = chicago_time.format(time_format).to_string();

    let format_indicator = if use_24_hour { "24h" } else { "12h" };
    let status_text = format!(
        " Local: {} | Chicago: {} | {} ",
        local_time_str, chicago_time_str, format_indicator
    );
